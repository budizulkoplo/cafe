...
<?php include 'header.php'; ?> <div class="col-12 col-md-12 p-3 p-md-3 bg-white border border-white text-center">
    ...
    <h3 class="text-center">LAPORAN TRANSAKSI HARIAN</h3>
    <hr>
    <div class="container">
        <div class="row">
...
<script>
    $(document).ready(function() {
        ...
        $("#tblharian").load("displayharian.php?cmd=display");

        // fungsi untuk memfilter data berdasarkan tanggal
        $('#tgllaporan').datepicker({
            dateFormat: 'yy-mm-dd',
            onSelect: function() {
                $("#tblharian").load("displayharian.php?cmd=display&tgl=" + $(this).val());
...
<script type="text/javascript">
    ...
    function exportToPdf() {
        ...
        window.open("harianpdf.php?tgl=" + $('input[name="tgllaporan"]').val() + "&storeid=" + $('select[name="storeid"]').val() + "&kategori=" + $('select[name="opsipesan"]').val() + "&bayar=" + $('select[name="opsibayar"]').val());
    ...
    function exportToXLS() {
        ...
        window.open("harianexcel.php?tgl=" + $('input[name="tgllaporan"]').val() + "&storeid=" + $('select[name="storeid"]').val() + "&kategori=" + $('select[name="opsipesan"]').val() + "&bayar=" + $('select[name="opsibayar"]').val());
