        ...
        <table class="table table-bordered shadow" width="100%" cellspacing="0">
            ...
            <tbody>
                ...
                if (in_array($_SESSION['user_role'], array("admin", "super admin"))) {
                    $displaystoreQuery = "select tgltransaksi, kategori,namatransaksi, jumlah, kasir, keterangan, jmlitem, hargasatuan, satuan,
                    ...
                    $displaystoreQuery = "select cast(tgltransaksi as date) tgltransaksi,concat(cast(tgltransaksi as date),kategori) as cator, kategori,namatransaksi, jumlah, kasir, keterangan, jmlitem, hargasatuan, satuan,
                    (select count(*) from tbl_operasional where 
                     tbl_operasional.tgltransaksi=TBO.tgltransaksi and tbl_operasional.storeid=TBO.storeid) rowspan1,
                    (select count(*) from tbl_operasional where tbl_operasional.kategori=TBO.kategori 
                ...
                if ($result = $sqlconnection->query($displaystoreQuery)) {
                    ...
                    while ($rs = $result->fetch_array(MYSQLI_ASSOC)) { ?>
                        ...
                        <tr align="left">
                            ...
                            <td><?php echo $rs['jmlitem'] . " " . $rs['satuan']; ?></td>
                            <td>Rp. <?php echo number_format($rs['hargasatuan'], 0, ',', '.'); ?></td>
...
<script type="text/javascript">
    ...
    function exportToPdf() {
        ...
        window.open("pengeluaranpdf.php?tanggal=" + $('input[name="tgllaporan"]').val() + "&tanggal2=" + $('input[name="tgllaporan2"]').val());
    ...
    function exportToXLS() {
        ...
        window.open("pengeluaranexcel.php?tanggal=" + $('input[name="tgllaporan"]').val() + "&tanggal2=" + $('input[name="tgllaporan2"]').val());
