...
header("Content-Disposition: attachment; filename=harian-" . $_GET['tgl'] . ".xls");
...
( SELECT count(itemID) FROM tbl_orderdetail WHERE tbl_orderdetail.orderID = c.orderID ) rowspan,
...
where  cast(order_date as date)='{$tgl}' and opsipesanan like '{$kategori}' and opsibayar like '{$bayar}'
...
<table>
    ...
    <tr>
        <td colspan='7'>Tanggal: {$tgl}</td> <td align='right' colspan='8'>Kasir: {$kasir}</td>
    </tr>
</table>
