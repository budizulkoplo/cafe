...
if (!empty($_POST['category'])) {
    ...
    $updatecatpengeluaran = "UPDATE tbl_store SET category = '{$category}'  WHERE storeID = {$storeID}  ";
    if ($sqlconnection->query($updatecatpengeluaran) === TRUE) {
        echo "";
    } else {
        echo "someting wong";
            ...
            <tbody>
                ...
                if ($result = $sqlconnection->query($displaystoreQuery)) {
                    ...
                    while ($store = $result->fetch_array(MYSQLI_ASSOC)) { ?> <tr>
                            ...
                            <!-- sampai sini -->
                            <td align="left"><?php echo $store['nama']; ?></td>
                            <td><input type="text" id="qty" name="jml" value="<?php echo $store['jml']; ?>" style="width: 80px;" onchange="updateJml(<?php echo $store['idinventory']; ?>, this.value)"></td>
                            ...
                            <td><input type="text" id="add" name="add" value="" style="width: 80px;" onchange="addJml(<?php echo $store['idinventory']; ?>, this.value)"></td>
