...
if (isset($_POST['status']) && isset($_POST['orderID'])) {
	...
	//bataltransaksi
	$cancelconsumption = "update tbl_consumption set status='cancelled'
	where orderid = {$orderID};";

	$cancelorderupdateinventory = "update tbl_inventory tbi set jml=(jml+(select inventoryqty*itemqty from tbl_consumption where tbl_consumption.idinventory=tbi.idinventory and tbl_consumption.orderID = {$orderID}))  
	where idinventory in(select idinventory from tbl_consumption where orderid={$orderID});";

	if ($sqlconnection->query($addOrderQuery) === TRUE) {
		if ($status == "cancelled") {
			$sqlconnection->query($cancelconsumption);
			$sqlconnection->query($cancelorderupdateinventory);
			echo "canceled.";
		}
	} else {
