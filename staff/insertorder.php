		...
		if (count($arrItemID) == count($arrItemQty)) {
			...
			if (mysqli_num_rows($resultcek) == 0) {
				...
				for ($i = 0; $i < $arrlength; $i++) {
					insertOrderDetailQuery($currentOrderID, $arrItemID[$i], $arrItemQty[$i], $arrItemPesan[$i]);
				}
...
function insertOrderDetailQuery($orderID, $itemID, $quantity, $pesan)
{
	global $sqlconnection;
	$addOrderQuery = "INSERT INTO tbl_orderdetail (orderID ,itemID ,quantity,pesan, hargajual, hargamodal) VALUES ('{$orderID}', '{$itemID}' ,{$quantity}, '{$pesan}', (select price from tbl_menuitem where itemID='{$itemID}' limit 1), (select price from tbl_menuitem where itemID='{$itemID}' limit 1))";
