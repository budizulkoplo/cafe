...
<div class="container">
  <div class="row">
    ...
    <div class="col-12 col-md-8 p-3 p-md-3 shadow rounded">
      ...
      <div id="tblcategory" class="container bg-light">
        <div class='menu col-12'><button class='btn' style='width:100%; background-color:#CCCCCC;' onclick='segomenu()'>
            <h6>SEGO BERKAT</h6>
          </button></div>
        <div id="menusego">
          <div class="row">
            <?php
            $menuQuery = "SELECT * FROM tbl_menu where storeid='1' and active='1' order by storeid, menuName  asc";
            if ($menuResult = $sqlconnection->query($menuQuery)) {
              while ($menuRow = $menuResult->fetch_array(MYSQLI_ASSOC)) {
            ?>
                <div class="menu col-6"><button class="btn" style='width:100%; border-color:#FF968A; background-color:#FF968A;' onclick="displayitemnew(<?php echo $menuRow['menuID'] ?>)">
        ...
        <div class='menu col-12'><button class='btn' style='width:100%; background-color:#CCCCCC;' onclick='santemenu()'>
            <h6>SAN.TE CAFE</h6>
          </button></div>
        <div id="menusante">
          <div class="row">
            <?php
            $menuQuery = "SELECT * FROM tbl_menu where storeid='2' and active='1'  order by storeid, menuName  asc";
            if ($menuResult = $sqlconnection->query($menuQuery)) {
              while ($menuRow = $menuResult->fetch_array(MYSQLI_ASSOC)) {
            ?>
                <div class="menu col-6"><button class="btn" style='width:100%; border-color:#F6EAC2; background-color:#F6EAC2;' onclick="displayitemnew(<?php echo $menuRow['menuID'] ?>)">
                    <?php echo $menuRow['menuName'] ?>
                  </button></div>
      ...
      <div class='menu col-12' id='carimenu'><input type='text' class='form-control' id='cari'></div>
      ...
      <div id="tblItem" class="container bg-light text-white">
      </div>
...
<script>
  $(document).ready(function() {
    document.getElementById("menusego").style.display = "none";
    document.getElementById("menusante").style.display = "none";
    document.getElementById("carimenu").style.display = "none";
  });
...
<script>
  ...
  function displayitemnew(id) {
    let myInput = document.getElementById("cari");
    //myInput.value = "";
    // myInput.focus();

    $.ajax({
      url: "displayitemnew.php",
      type: 'POST',
      data: {
        btnMenuID: id
      },
      success: function(output) {
        document.getElementById("tblcategory").style.display = "none";
        document.getElementById("tblItem").style.display = "block";
        document.getElementById("carimenu").style.display = "block";
        $("#tblItem").html(output);
        // $("#tblcategory").html(hide);
        calculateTotal()
      }
    });
  }

  function backmenu(id) {

    document.getElementById("tblcategory").style.display = "block";
    document.getElementById("tblItem").style.display = "none";
    document.getElementById("carimenu").style.display = "none";
    calculateTotal()
  }

  function segomenu() {
    document.getElementById("menusego").style.display = "block";
    document.getElementById("menusante").style.display = "none";
    calculateTotal()
  }
  ...
  function santemenu() {
    document.getElementById("menusego").style.display = "none";
    document.getElementById("menusante").style.display = "block";
    calculateTotal()
  }
