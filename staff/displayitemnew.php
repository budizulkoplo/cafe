...
if (isset($_POST['btnMenuID'])) {
	...
	$menuItemQuery = "SELECT itemID,menuItemName, price, menuimg FROM tbl_menuitem WHERE active='1' and itemID not in(select idmenu from tbl_inventory j join tbl_detailinventory i
  ...
  where jml-(qty+(ifnull(itemqty,0)*ifnull(inventoryqty,0)))<0) and menuID = " . $menuID . " order by menuItemname asc";
	...
	$menuItemQueryhabis = "SELECT itemID,menuItemName, price, menuimg FROM tbl_menuitem WHERE active='1' and itemID in(select idmenu from tbl_inventory j join tbl_detailinventory i
on j.idinventory=i.idinventory
left join tbl_consumption a on a.idinventory=j.idinventory and `status`='waiting'
where jml-(qty+(ifnull(itemqty,0)*ifnull(inventoryqty,0)))<0) and menuID = " . $menuID . " order by menuItemname asc";
	...
	if ($menuItemResult = $sqlconnection->query($menuItemQuery)) {
		if ($menuItemResult->num_rows > 0) {
			$i = 0;
			echo "
			<div  class='menu col-12'><button class='btn' style='width:100%; background-color:#CCCCCC;' onclick='backmenu()'><h6>KEMBALI</h6></button></div>";
			while ($menuItemRow = $menuItemResult->fetch_array(MYSQLI_ASSOC)) {
				$i++;
				echo "<div id='btnmenu[{$i}]' class='menu col-6'><button class='btn' style='width:100%; background-color:#FFF9F2; border-color:#FF8000; ' onclick = 'setQty({$menuItemRow['itemID']})'>	
			<div class='image-menu'>
			<img src='../image/menu/{$menuItemRow['menuimg']}' alt='Gambar' />
			</div>
			<div class='menu-text'>
			{$menuItemRow['menuItemName']}<br>
			Rp. " . number_format($menuItemRow['price'], 0, '', '.') . "</div>
			</div></button>";
			}
		} else {
...
if ($menuItemResulthabis = $sqlconnection->query($menuItemQueryhabis)) {
	if ($menuItemResulthabis->num_rows > 0) {
		...
		while ($menuItemRow = $menuItemResulthabis->fetch_array(MYSQLI_ASSOC)) {
			$i++;
			echo "<div id='btnmenu[{$i}]' class='menu col-6'><button disabled class='btn' style='width:100%; background-color:#F0F0F0; border-color:#FF8000; ' onclick = 'setQty({$menuItemRow['itemID']})'>
			<div class='image-menu'>
			<img src='../image/menu/{$menuItemRow['menuimg']}' alt='Gambar' />
			</div>
			<div class='menu-text'>
