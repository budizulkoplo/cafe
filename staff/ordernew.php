...
<div class="container">
  <div class="row">
    ...
    <div class="col-12 col-md-8 p-3 p-md-3 shadow rounded">
      ...
      <div  class='menu col-12'><button class='btn text-white' style='width:100%; background-color:#A85D0A;' onclick='segomenu()'><h6>SEGO BERKAT</h6></button></div>
      <div id="menusego">
        <div class="row">
          <?php
          $menuQuery = "SELECT * FROM tbl_menu where storeid='1' order by storeid, menuName  asc";
          if ($menuResult = $sqlconnection->query($menuQuery)) {
            while ($menuRow = $menuResult->fetch_array(MYSQLI_ASSOC)) {
          ?>
              <div class="menu col-6"><button class="btn" style='width:100%; border-color:#A85D0A;' onclick="displayitemnew(<?php echo $menuRow['menuID'] ?>)">
        ...
        <div  class='menu col-12'><button class='btn text-white' style='width:100%; background-color:#A85D0A;' onclick='santemenu()'><h6>SAN.TE CAFE</h6></button></div>
        <div id="menusante" >
        <div class="row">
          <?php
          $menuQuery = "SELECT * FROM tbl_menu where storeid='2' order by storeid, menuName  asc";
          if ($menuResult = $sqlconnection->query($menuQuery)) {
            while ($menuRow = $menuResult->fetch_array(MYSQLI_ASSOC)) {
          ?>
              <div class="menu col-6"><button class="btn" style='width:100%; border-color:#A85D0A;' onclick="displayitemnew(<?php echo $menuRow['menuID'] ?>)">
