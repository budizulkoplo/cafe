...
if (isset($_POST["ambil"])) {
	if ($_POST["ambil"] == 'komposisiawal') {
		...
		$str = "SELECT * FROM tbl_inventory WHERE idstore='$storeid' and idinventory NOT in (";
...
?>
	...
	if ($_POST["ambil"] == 'editkomposisi') {
		$storeid = $_POST['storeid'];
		$itemid = $_POST['menuid'];
		$str = "SELECT * FROM tbl_inventory WHERE idstore='$storeid' and idinventory not in(select idinventory from tbl_detailinventory where idmenu='" . $itemid . "') and idinventory NOT in (";
		if (isset($_POST['komposisi'])) {
